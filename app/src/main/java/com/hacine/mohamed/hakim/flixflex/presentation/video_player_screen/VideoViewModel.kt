...
@HiltViewModel
class VideoViewModel @Inject constructor(private val moviesRepository: MoviesRepository) :
    ViewModel() {
    ...
    fun getSeriesVideos(movieid: String) {
        viewModelScope.launch {
            Log.d("getMovieById", "getMovieById: ")
            _movieDetailuiState.value = UiState.Loading
            when (val response = moviesRepository.getSeriesVideos(movieid)) {
                is Resource.Success -> {
                    _movieDetailuiState.value = UiState.Success(data = response.value)
                    Log.d("getMovieById", "getMovieById: " + response.value)
                }
